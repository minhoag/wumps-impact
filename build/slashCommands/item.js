...
const command = {
    ...
    autocomplete: async (interaction) => {
        if (interaction.options.getSubcommand() === 'add') {
            ...
            try {
                ...
                const filtered = item_1.item.filter((choice) => choice.name.toLowerCase().includes(focusedOption.value.toLowerCase()));
    ...
    execute: async (interaction) => {
        ...
        if (interaction.options.getSubcommand() === 'add') {
            ...
            if (!interaction.guild)
                return interaction.reply('Không thể thực hiện ở DM');
            const itemId = item_1.item.find((e) => e.value === id) ?? item_1.item.find((e) => e.value === id);
            if (!itemId)
            ...
            try {
                await fetch(`http://${ip}:14861/api?region=dev_gio&ticket=GM&cmd=1116&uid=${uid}&msg=item%20add%20${itemId.value}%20${amount}`);
            ...
            try {
                await fetch(`http://${ip}:14861/api?region=dev_gio&ticket=GM&cmd=1116&uid=${uid}&msg=item%20clear%20${itemId.value}%20${amount}`);
                await interaction.reply({
            ...
            const item = {
                ...
                mainStat: params[3],
                subStat: (0, function_1.extractSubstats)(params.slice(4).join(' ')) || null,
            };
            if (!interaction.guild)
                return interaction.reply('Không thể thực hiện ở DM');
            try {
                const uuid = new Date().getTime();
                const res = await fetch(`http://${ip}:14861/api?uid=${uid}&item_id=${item.itemId}&cmd=1127&item_count=${item.quantity}&extra_params={"level":%20${item.level + 1},"exp":%200,"main_prop_id":%20${item.mainStat},"append_prop_id_list":[${item.subStat}]}&region=dev_gio&ticket=GM@${uuid}&sign=${uuid}`);
