...
export const SUCCESS_MESSAGE: {
    ...
    3000: {
        [Locale.Vietnamese]: 'Thành công `{action}` `{itemName}` cho người chơi `{playerName}`. Số lượng: `{quantity}`.',
        [Locale.EnglishUS]: 'Successfully `{action}` `{itemName}` for player `{playerName}`. Quantity: `{quantity}`.',
    }
}
