	...
	async sendResponse() {
		...
		for (let i: number = 0; i < this.item.length / 3; i++) {
			const embed: EmbedBuilder = new EmbedBuilder()
				...
				.setFooter({
					text: Locale['buy:moraFooter'][locale],
					iconURL:
		...
		if (interaction.options.getSubcommand() === 'shop') {
			...
			collector.on('collect', async i => {
				if (i.customId === 'shopCredit') {
					...
					i.reply({
						content: `${locale == 'vi' ? 'Shop tạm thời không khả dụng' : 'This shop is not yet available'}`,
						ephemeral: true
					})
			...
			try {
				...
				if (confirmation.customId === 'confirm') {
					...
					if (userData.mora - BigInt(finalPrice) < 0) return await confirmation.update({
						content: Locale['buy:notenoughMora'][locale],
						embeds: [],
						components: []
					})
					...
					await fetch(
						...
						await confirmation.update({ content: `${Locale['buy:success'][locale]} ${error.name}: ${error.message}` })
					})
				} else if (confirmation.customId === 'cancel') {
					await confirmation.update({
						content: Locale['buy:cancel'][locale],
						embeds: [],
						components: []
					})
				}
				...
				await interaction.editReply({
					content: Locale['buy:timeout'][locale],
					embeds: [],
					components: []
				})
			}
			...
			try {
				...
				if (confirmation.customId === 'confirm') {
					...
					if (userData.points - finalPrice < 0) return await confirmation.update({
						content: Locale['buy:notenoughPoint'][locale],
						embeds: [],
						components: []
					})
					await prisma_sqlite.userData.update({
					...
					await sendLog(interaction, `<@${interaction.user.id}> has bought **${quantity} x ${price.image}}**`)
				} else if (confirmation.customId === 'cancel') {
					await confirmation.update({
						content: Locale['buy:cancel'][locale],
						embeds: [],
