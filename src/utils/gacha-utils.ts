...
export class GachaUtils {
  ...
  private getGachaUpConfig (gachaData: any): string {
    ...
    const items4Array = rateUpItems4 ? rateUpItems4.split(',').map((id: string) => parseInt(id.trim(), 10)) : [];
    const items5Array = rateUpItems5 ? rateUpItems5.split(',').map((id: string) => parseInt(id.trim(), 10)) : [];
  ...
  public async create(): Promise<boolean> {
    ...
    const rateUpItems4 = gachaData.rateUpItems4;
    const createdGacha = await ConfigPrisma.t_gacha_schedule_config.create({
      data: {
        ...
        display_up4_item_list: rateUpItems4 ? rateUpItems4.split(',').map((id: string) => parseInt(id.trim(), 10)).join(',') : ''
      },
    });
  ...
  public async update(): Promise<boolean> {
    const updatedGacha = await ConfigPrisma.t_gacha_schedule_config.update({
  ...
  public async delete(): Promise<boolean> {
    const deletedGacha = await ConfigPrisma.t_gacha_schedule_config.delete({
      where: { schedule_id: this.options.schedule_id },
    });
